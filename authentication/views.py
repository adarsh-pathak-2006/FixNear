    ...
    def post(self, request):
        ...
        if serial.is_valid():
            ...
            if User.objects.filter(
                Q(username=username) | Q(email=email) | Q(mobile_no=mobile_no)
            ).exists():
                return Response(
                    {'message': 'username, email or mobile_no already exists'},
                    status=400,
                )
...
class MyTechnicianProfile(APIView):
    ...
    def get(self, request):
        ...
        if cached_data:
            return Response(cached_data, status=200)
        data = get_object_or_404(
            TechnicianProfile.objects.select_related('user'), user=request.user
        )
...
class TechnicianProfileListAPI(APIView):
    ...
    def get(self, request):
        ...
        data = paginator.paginate_queryset(
            TechnicianProfile.objects.select_related('user').filter(is_available=True),
            request,
            view=self,
        )
